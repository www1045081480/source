...
//仕入纳期控件
//@param   naqi   1 为纳期控件  2为invoice NO DATE联动控件  3为L/C DATE联动控件 4为Issued by DATE联动控件 5为Sailing on or about DATE联动控件
//@param   naqi   6 为未注文一览时间段查询
function setday1(naqi,inputObj,dateFormatStyle,beginDate,endDate,lang){
	loadtime = inputObj.value;
	if(null == inputObj){return null;}
	if(naqi == 1){naqidate = 1;}
	if(naqi == 2){naqidate = 2;}
	if(naqi == 3){naqidate = 3;}
	if(naqi == 4){naqidate = 4;}
	if(naqi == 5){naqidate = 5;}
	if(naqi == 6){naqidate = 6;}
	new Calendar(inputObj,dateFormatStyle,beginDate,endDate,lang).show();
...
Calendar.prototype.setAutoHeight = function(){
	...
	//日历所在的iframe
	this.iframe.style.height = "184px";
	this.panel.style.height = "184px";
...
//纳期变更的时候用
Calendar.prototype.hide1 = function() {
  this.setDisplayStyle("select", "visible");
  this.panel.style.visibility = "hidden";
  this.isFocus = false;

  //alert("确定要更改吗？");
  //alert("确定将日期变更为"+this.dateControl.value);
  //document.getElementById("testtime").value=this.dateControl.value;
  var nowdate = this.dateControl.value;
  var ch = new Array;
  ch = nowdate.split("-");
  cMon = ch[1];
  var sh = new Array;
  sh = loadtime.split("-");
  sMon = sh[1];
   if (confirm("确定将日期变更为"+this.dateControl.value)) { 
	   //alert(this.dateControl);
	   //this.dateControl为当前对象下面操作是删除这条信息
	  if(cMon!=sMon){
		  var eparent = this.dateControl.parentNode;
		...
		//点击取消日期恢复原始信息
		this.dateControl.value=loadtime;
		//alert(this);
           // alert("点击了取消");  
    };
  //alert(123);
  //document.getElementById("dtime2").value=this.dateControl.value;
  //alert(123);
  //alert(calendar.date);
}
//invoice NO DATE联动时候用
Calendar.prototype.hide2 = function() {
	  this.setDisplayStyle("select", "visible");
	  this.panel.style.visibility = "hidden";
	  this.isFocus = false;
	  //alert(this.dateControl.value);
	  document.getElementById("ptoday").value = this.dateControl.value;
	  document.getElementById("dtoday").value = this.dateControl.value;
}
//  L/C DATE联动时候用
Calendar.prototype.hide3 = function() {
	  this.setDisplayStyle("select", "visible");
	  this.panel.style.visibility = "hidden";
	  this.isFocus = false;
	  //alert(this.dateControl.value);
	  document.getElementById("plcday").value = this.dateControl.value;
}
//Issued by DATE联动时候用
Calendar.prototype.hide4 = function() {
	  this.setDisplayStyle("select", "visible");
	  this.panel.style.visibility = "hidden";
	  this.isFocus = false;
	  //alert(this.dateControl.value);
	  document.getElementById("disday").value = this.dateControl.value;
}
//Sailing on or about DATE联动时候用
Calendar.prototype.hide5 = function() {
	  this.setDisplayStyle("select", "visible");
	  this.panel.style.visibility = "hidden";
	  this.isFocus = false;
	  //alert(this.dateControl.value);
	  document.getElementById("dsaday").value = this.dateControl.value;
}
//为未注文一览时间段查询
Calendar.prototype.hide6 = function() {
	  this.setDisplayStyle("select", "visible");
	  this.panel.style.visibility = "hidden";
	  this.isFocus = false;
	  //alert(this.dateControl.value);
	 ...
	 if(Form!=""){
		 var ch = new Array;
		 ch = Form.split("-");
		 var form =  ch[0]+ch[1]+ch[2];
	 }
	 if(To!=""){
		 var ah = new Array;
		 ah = To.split("-");
		 var to =  ah[0]+ah[1]+ah[2];
	 }
	 if(Form==""){
		 alert("请填写开始时间");
		 return;
	 ...
	 if(parseInt(form)>=parseInt(to)){
	  ...
	  //alert(form);
	 // alert(to);
	  var urlstr = "xinan/noOrderEstimation.action?Form="+form+"&To="+to;
	  //alert(urlstr);
//	  $.ajax({
//			type:"POST",
//			url:urlstr,
...
//				alert("查询成功");
//				document.getElementById("nolist").value = "noOrderEstimation";
//			}
//		});
...
document.write('<div id="calendarPanel"  allowtransparency="true" style="position:absolute;visibility:hidden;z-index:9999;font-size:12px;width:20px;">');
