...
function save(){
	...
	if(jpname == "" || jpname.indexOf("(例)")==0){
		alert("会社名（日本語）を入力してください！");
		return false;
	}
	if(cnname == "" || cnname.indexOf("(例)")==0){
		alert("会社名（中国語）を入力してください！");
		return false;
	}
	if(enname == "" || enname.indexOf("(例)")==0){
		alert("会社名（英語）を入力してください！");
		return false;
	}
	...
	if(enaddress1 == "" || enaddress1.indexOf("(例)")==0){
		alert("住所-英語（県、市、区）を入力してください！");
		return false;
	}
	if(enaddress2 == "" || enaddress2.indexOf("(例)")==0){
		alert("住所-英語（町）を入力してください！");
		return false;
	}
	if(enaddress3 == "" || enaddress3.indexOf("(例)")==0){
		alert("住所-英語（巨細）を入力してください！");
		return false;
	}
	...
	$.ajax({
	    ...
	    success: function(data) {
	    	...
	    	reset();
	    	findAll();
//	    	 location.href = "xinan/supplier.action";
	    }
	});
}
//查询
function find(){
    ...
    $.ajax({
		type:"post",
		url:"xinan/searchSupplier.action?",
		async:"false",
		data:urlstr,
		dataType:"json",
		error:function() {  
...
//			var len = data.length;
			...
			for(var i=0;i<data.length;i++){
				...
				if(data[i].fax){
				 ...
				 th = th + "<tr>"+
				 		"<td style='display: none'>"+data[i].supplierId+"</td>"+
				 		"<td >"+jpName+"</td>"+
				 		"<td >"+cnName+"</td>"+
				 		"<td >"+enName+"</td>"+
				 		"<td >"+enShortName+"</td>"+
...
//	 location.href = "xinan/searchSupplier.action?" + urlstr;
}
//
function dele(evt){
	if(confirm("仕入先は削除しますか？")){
	var eparent = evt.parentNode;
    var tparent = eparent.parentNode;
    var supplierid = tparent.getElementsByTagName('td')[0].innerHTML;
    $.ajax({
		type:"post",
		url:"xinan/deleSupplier.action?supplierid="+supplierid,
		async:"false",
		dataType:"text",
		error:function() {  
			alert('削除しませんでした');   
...
function change(evt){
    ...
    document.getElementById("supplierid").value = tparent.getElementsByTagName('td')[0].innerHTML;
