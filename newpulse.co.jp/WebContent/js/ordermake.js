...
function OrderSheet(e,reqStr){
	...
	$.ajax({
		...
		async:"true",
		data:{"EstimationId":e,"reqStr":reqStr,"lanCd":'En'},
		dataType:"json",
		error:function(){
			alert("error");
		}, 
		success:function(data){
			...
			var $table= $("#sptable");
			var tableStr = "<tr id='thead'><td width='50px'>No.</td>" + 
			"<td width='115px'>Description</td><td width='115px'>Specification</td><td width='54px'>Quantity</td>"+
			"<td width='30px'>Unit</td><td width='80px'>Unit Price</td>"+
			"<td width='80'>Total Amount</td><td width='90px'>Delivery Date</td>" +
...
function changeSubtd(){
		...
		document.getElementById("discount").innerHTML=count;
		$.ajax({
					type:"POST",
...
//						}
						...
						$("#address1").val(data[0].enaddress1);
						$("#address2").val(data[0].enaddress2);
						$("#address3").val(data[0].enaddress3);
